...
- Use `just tui-ttyglass` as the standard visual verification path when changing the TUI layout, colors, styling, navigation, keyboard interactions, or visual behavior. Inspect the real terminal session in ttyglass and capture a screenshot for visual changes; use ttyglass diagnostics when investigating behavior.
